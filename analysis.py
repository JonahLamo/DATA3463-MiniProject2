...
# Figure 5: Top 15 Countries by Medal Count
def fig_top_countries(df):
    ...
    save(fig, "05_top_countries_medals")


# Figure 6: National GDP vs Olympic Medal Count
def fig_gdp_vs_medals(df, countries):
    ...
    save(fig, "06_gdp_vs_medals")


# Figure 7: Medal Efficiency - Medals per Capita
def fig_medals_per_capita(df, countries):
    ...
    ax.invert_yaxis()
    save(fig, "07_medals_per_capita")


# Figure 8: Medal Trajectories by Country
def fig_medal_timeseries(df):
    ...
    fig.suptitle("Medal Trajectories by Country (2010 - 2026)",
                 fontsize=14, fontweight="bold", x=0.0, ha="left", y=1.01)
    fig.tight_layout()
    save(fig, "08_medal_timeseries")


# Figure 9: Medalist Height Distribution by Sport
def fig_height_by_sport(df):
    ...
    fig.tight_layout()
    save(fig, "09_height_by_sport")


# Figure 10: Medal Composition by Country (Heatmap)
def fig_gold_ratio_heatmap(df):
    ...
    save(fig, "10_gold_ratio_heatmap")


# Figure 11: Population vs Medal Count
def fig_population_vs_medals(df, countries):
    ...
    save(fig, "11_population_vs_medals")


# Figure 12: Correlation Summary Dashboard
def fig_correlation_dashboard(df, countries):
    ...
    fig.suptitle("Correlation Summary: What Predicts Medal Success?",
                 fontsize=14, fontweight="bold", x=0.0, ha="left", y=1.01)
    fig.tight_layout()
    save(fig, "12_correlation_dashboard")


# Figure 13: R² Heatmap by Sport
def fig_r2_heatmap(df, countries):
    ...
    fig.tight_layout()
    save(fig, "13_r2_heatmap_by_sport")
    return r2_df, n_countries, factor_cols


# Figure 14: R² Bars by Sport
def fig_r2_bars(r2_df, factor_cols):
    ...
    fig.tight_layout()
    save(fig, "14_r2_bars_by_sport")


# Figure 15: Medalist Weight Distribution by Sport
def fig_weight_by_sport(df):
    ...
    ax.tick_params(axis="x", rotation=40)
    fig.tight_layout()
    save(fig, "15_weight_by_sport")


# Figure 16: Medalist Age Distribution by Sport
def fig_age_by_sport(df):
    ...
    ax.tick_params(axis="x", rotation=40)
    fig.tight_layout()
    save(fig, "16_age_by_sport")


# Figure 17: Top Medal-Producing Cities by Sport
def fig_top_cities(df):
    ...
    ax.invert_yaxis()
    fig.tight_layout()
    save(fig, "17_top_cities_by_sport")


# Figure 18: How Medalist Physiques Have Changed Over Time
def fig_physique_trends(df):
    ...
    fig.tight_layout()
    save(fig, "18_physique_trends")


# Figure 19: Medalist Physique Trends by Sport (small multiples)
def fig_physique_trends_by_sport(df):
    ...
    save(fig, "19_physique_trends_by_sport")


# Figure 20: Medal Efficiency by Sport
def fig_medal_efficiency(df):
    ...
    for _, row in sport_stats.iterrows():
        ax.text(row["countries"] + 0.2, row["total_medals"], row["sport"], fontsize=7, color=DARK)

    ax.set_xlabel("Number of Competing Countries")
    ax.set_ylabel("Total Medals Awarded")
    ax.set_title("Medal Efficiency by Sport", fontsize=14, fontweight="bold", pad=12)
    save(fig, "20_medal_efficiency")


# Figure 21: Gold Conversion Rate by Country
def fig_gold_conversion(df):
    medal_cts = df.groupby(["Country", "medal"]).size().unstack(fill_value=0)
    medal_cts = medal_cts.reindex(columns=MEDAL_ORDER, fill_value=0)
    medal_cts["total"] = medal_cts.sum(axis=1)
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.barh(top.index, top["gold_ratio"], color=GOLD, edgecolor="white", height=0.6)

    ax.set_xlabel("Gold Medal Share")
    ax.set_title("Gold Conversion Rate by Country", fontsize=14, fontweight="bold", pad=12)
    ax.xaxis.set_major_formatter(mticker.PercentFormatter(1.0))
    save(fig, "21_gold_conversion")


# Figure 22: How Competitive is Each Sport?
def fig_medal_concentration(df):
    ...
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.barh(conc["sport"], conc["top3_share"], color=BRONZE, edgecolor="white", height=0.6)

    ax.set_xlabel("Share of Medals Won by Top 3 Countries")
    ax.set_title("How Competitive is Each Sport?", fontsize=14, fontweight="bold", pad=12)
    ax.xaxis.set_major_formatter(mticker.PercentFormatter(1.0))
    save(fig, "22_medal_concentration")


# Figure 23: Which Countries Overperform Their Wealth?
def fig_gdp_efficiency(df, countries):
    ...
    ax.scatter(cm["gdpPerCapita"], cm["medals_per_M"],
               s=70, c=SILVER, edgecolors=DARK, alpha=0.8, linewidth=0.5)

    for _, row in cm.nlargest(5, "medals_per_M").iterrows():
        ax.annotate(row["Country"],
                    (row["gdpPerCapita"], row["medals_per_M"]),
                    fontsize=8, color=DARK, xytext=(5, 4),
...
# Figure 24: Do Top Countries Have Physique Advantages?
def fig_physique_advantage(df):
    sub = df.dropna(subset=["height"]).copy()

    global_means = sub.groupby("sport")["height"].mean()
    top = df.groupby("country").size().nlargest(TOP_N).index
    ...
    fig, ax = plt.subplots(figsize=(9, 6))
    ax.barh(diff.index, diff.values, color=GOLD, edgecolor="white", height=0.6)
...
# Figure 25: Fastest Improving Countries
def fig_medal_growth(df):
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.barh(labels, top["growth"].values, color=ACCENT, edgecolor="white", height=0.6)

    ax.set_xlabel("Change in Medal Count (2010 to 2026)")
    ax.set_title("Fastest Improving Countries", fontsize=14, fontweight="bold", pad=12)
